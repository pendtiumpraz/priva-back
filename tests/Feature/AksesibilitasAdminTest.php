...
class AksesibilitasAdminTest extends TestCase
{
    ...
    private function pengguna(string $role, ?string $divisi, array $izin = ['consent:read', 'consent:write']): User
    {
    ...
    public function prasarana_yang_tersedia_tanpa_tanggal_uji_belum_terbukti(): void
    {
        ...
        $r = $this->postJson('/api/accessibility/provisions', [
        ...
        $this->putJson('/api/accessibility/provisions/'.$id, ['last_tested_at' => now()->toDateString()])
            ->assertOk()
            ->assertJsonPath('data.sudah_terbukti', true);

        // Duplikat (kanal, format) ditolak — perbarui barisnya, jangan gandakan.
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget Consent', 'format' => AccessibilityProvision::FORMAT_TTS])
            ->assertStatus(409)->assertJsonPath('code', 'SUDAH_ADA');

        // Format di luar kosakata ditolak.
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget Consent', 'format' => 'telepati'])
            ->assertStatus(422)->assertJsonValidationErrors(['format']);

        $this->getJson('/api/accessibility/provisions')->assertOk()->assertJsonCount(1, 'data');

        $this->deleteJson('/api/accessibility/provisions/'.$id)->assertOk();
        $this->getJson('/api/accessibility/provisions')->assertOk()->assertJsonCount(0, 'data');
    }
    ...
    public function config_publik_hanya_memaparkan_prasarana_yang_terbukti_dan_langsung_segar(): void
    {
        ...
        $terbukti = $this->postJson('/api/accessibility/provisions', [
        ...
        $this->postJson('/api/accessibility/provisions', [
            'channel' => 'Widget Pelajar', 'collection_point_id' => $cp->id,
            'format' => AccessibilityProvision::FORMAT_BRAILLE, // tersedia, tapi tidak pernah diuji
        ])->assertStatus(201);

        $this->postJson('/api/accessibility/scopes', [
        ...
        $this->putJson('/api/accessibility/provisions/'.$terbukti, ['is_available' => false])->assertOk();
        $this->getJson('/api/public/consent/config?collection_id='.$cp->collection_id)
            ->assertOk()->assertJsonCount(0, 'data.accessibility.formats');
    }
    ...
    public function ragam_mengetahui_siapa_yang_boleh_menyetujui_sendiri(): void
    {
        ...
        $this->postJson('/api/accessibility/scopes', ['channel' => 'Loket Cabang', 'ragam' => DisabilityServiceScope::RAGAM_MENTAL])
            ->assertStatus(201)->assertJsonPath('data.boleh_mandiri', false);
        $this->postJson('/api/accessibility/scopes', ['channel' => 'Loket Cabang', 'ragam' => DisabilityServiceScope::RAGAM_NETRA])
            ->assertStatus(201)->assertJsonPath('data.boleh_mandiri', true);

        $this->postJson('/api/accessibility/scopes', ['channel' => 'Loket Cabang', 'ragam' => DisabilityServiceScope::RAGAM_NETRA])
            ->assertStatus(409)->assertJsonPath('code', 'SUDAH_ADA');
        $this->postJson('/api/accessibility/scopes', ['channel' => 'Loket Cabang', 'ragam' => 'telepati'])
            ->assertStatus(422)->assertJsonValidationErrors(['ragam']);

        $this->getJson('/api/accessibility/scopes')->assertOk()->assertJsonCount(2, 'data');
    }
    ...
    public function penilaian_kapasitas_wajib_beralasan_dan_membuat_baris_subjek(): void
    {
        ...
        $this->postJson('/api/accessibility/assessments', [
        ...
        $this->getJson('/api/accessibility/assessments?subject='.urlencode('X@Contoh.ID'))
            ->assertOk()->assertJsonCount(1, 'data');
        $this->getJson('/api/accessibility/assessments?subject=lain@contoh.id')
    ...
    public function staf_divisi_melihat_prasarana_titik_divisinya_dan_kanal_umum_saja(): void
    {
        ...
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget HR', 'collection_point_id' => $hr->id, 'format' => AccessibilityProvision::FORMAT_TTS])->assertStatus(201);
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget Keuangan', 'collection_point_id' => $keuangan->id, 'format' => AccessibilityProvision::FORMAT_TTS])->assertStatus(201);
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Loket Cabang', 'format' => AccessibilityProvision::FORMAT_ISYARAT])->assertStatus(201);

        Sanctum::actingAs($this->pengguna('maker', 'HR'));
        $r = $this->getJson('/api/accessibility/provisions')->assertOk()->assertJsonCount(2, 'data');
        ...
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget Keuangan', 'collection_point_id' => $keuangan->id, 'format' => AccessibilityProvision::FORMAT_BRAILLE])
            ->assertStatus(422)->assertJsonValidationErrors(['collection_point_id']);

        Sanctum::actingAs($this->pengguna('admin', null));
        $this->getJson('/api/accessibility/provisions')->assertOk()->assertJsonCount(3, 'data');
    }
    ...
    public function tanpa_izin_consent_ditolak(): void
    {
        ...
        $this->getJson('/api/accessibility/summary')->assertStatus(403);

        Sanctum::actingAs($this->pengguna('maker', null, ['consent:read']));
        $this->getJson('/api/accessibility/summary')->assertOk();
        $this->postJson('/api/accessibility/provisions', ['channel' => 'X', 'format' => AccessibilityProvision::FORMAT_TTS])->assertStatus(403);
        $this->postJson('/api/accessibility/assessments', ['subject' => 'x@contoh.id', 'result' => 'mampu', 'reason' => 'cukup panjang untuk lolos'])->assertStatus(403);
    }
    ...
    public function ringkasan_membedakan_terbukti_dan_kedaluwarsa(): void
    {
        ...
        $this->postJson('/api/accessibility/provisions', ['channel' => 'Widget', 'format' => AccessibilityProvision::FORMAT_BRAILLE])->assertStatus(201);
        $this->postJson('/api/accessibility/scopes', ['channel' => 'Widget', 'ragam' => DisabilityServiceScope::RAGAM_NETRA])->assertStatus(201);
        $this->postJson('/api/accessibility/assessments', ['subject' => 'a@contoh.id', 'result' => 'mampu', 'reason' => 'Memahami penjelasan dengan baik.'])->assertStatus(201);
        $this->postJson('/api/accessibility/assessments', ['subject' => 'b@contoh.id', 'result' => 'diwakili_wali', 'reason' => 'Tidak dapat memahami penjelasan.'])->assertStatus(201);

        $this->getJson('/api/accessibility/summary')
            ->assertOk()
