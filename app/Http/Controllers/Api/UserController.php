...
class UserController extends Controller
{
    ...
    {
        $user = $request->user();
        // Route is auth-gated; preserve prior lenient behavior for a missing user.
        if (! $user) {
            return null;
        }

        if (app(PermissionService::class)->allows($user, 'users', $action)) {
            return null;
        }

        $message = $action === 'write'
            ? 'Akses ditolak — role Anda tidak memiliki izin write untuk User Management.'
            : 'Akses ditolak — role Anda tidak memiliki izin untuk User Management.';
    ...
    {
        if ($denied = $this->checkPerm($request, 'read')) {
            return $denied;
        }
        $auth = $request->user();
    ...
    {
        if ($denied = $this->checkPerm($request, 'write')) {
            return $denied;
        }
        $auth = $request->user();
        ...
        if ((in_array($auth->role, ['root', 'superadmin'], true) || $auth->role === 'admin') && $validated['role'] === 'admin' && empty($validated['org_id'])) {
            ...
            $orgSlug = $request->input('org_slug', Str::slug($orgName.'-'.uniqid()));
            ...
            if ($auth->role === 'admin') {
                if (! $parentId) {
                    $parentId = $auth->org_id;
                } // Default to their org
            }
        ...
        try {
            NotificationPreferenceDefaults::seedForUser($user);
        } catch (\Exception $e) {
